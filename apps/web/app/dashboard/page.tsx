...
export default async function DashboardPage() {
    ...
    const chartData = completedAudits.map((a: { createdAt: Date, score: number | null }) => ({
        date: new Date(a.createdAt).toLocaleDateString('en-US', { weekday: 'short' }),
        score: a.score,
    })).slice(-20);
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            ...
            <section className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <StatCard title="Avg Performance" value={String(avgScore)} change={avgScore > 0 ? "Active" : "No data"} changeLabel="" icon={<TrendingUp className="h-4 w-4" />} iconColor="text-emerald-400" iconBg="bg-emerald-500/10" />
                <StatCard title="Total Scans" value={String(totalScans)} change="All time" changeLabel="" icon={<Globe className="h-4 w-4" />} iconColor="text-blue-400" iconBg="bg-blue-500/10" />
                <StatCard title="Critical Issues" value={String(criticalIssues)} change="Needs attention" changeLabel="" icon={<AlertTriangle className="h-4 w-4" />} iconColor="text-rose-400" iconBg="bg-rose-500/10" />
                <StatCard title="Uptime" value="100%" change="Operational" changeLabel="" icon={<Timer className="h-4 w-4" />} iconColor="text-amber-400" iconBg="bg-amber-500/10" />
            </section>
            ...
            <section className="grid gap-6 lg:grid-cols-7">
                <Card className="lg:col-span-4 border-slate-800/60 bg-slate-900/40 backdrop-blur-sm">
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <CardDescription className="text-slate-400">Recent audit scores</CardDescription>
                            </div>
                    ...
                    <CardContent className="pl-2 h-[320px] flex items-center justify-center text-slate-500">
                        <p>Chart data ready: {chartData.length} points (Client Component required)</p>
                    </CardContent>
                </Card>
    ...
    return (
        <Card className="border-slate-800/60 bg-slate-900/40 backdrop-blur-sm hover:bg-slate-900/60 hover:border-slate-700/60 transition-all duration-200 cursor-pointer group">
            ...
            <CardContent>
                ...
                <p className="text-xs text-slate-500 mt-1.5"><span className="text-emerald-400 font-medium">{change}</span>{changeLabel && ` ${changeLabel}`}</p>
            </CardContent>
        </Card>
    );
}
